...
def time_stamp_minutes(t):
    temp = "" + t
    startTime = temp[0:8]
    hours = int(startTime[0:2])
    minutes = int(startTime[3:5]) + (hours * 60)
    return "" + str(minutes) + startTime[5:]

def time_stamp_hours(t):
    temp = "" + t
    startTime = temp[0:8]
    hours = int(startTime[0:2])
    minutes = int(startTime[3:5])
    return "" + str(hours) + ":" + str(minutes) + startTime[5:]
...
def is_time_stamp(l):
    ...
    if l[:2].isnumeric() and l[2] == ':':
        if(tSwitch):
            #timeStamp = time_stamp_minutes(l) + "\t"
            timeStamp = time_stamp_hours(l) + "\t"
            tSwitch = False
        return True
    return False
...
def clean_up(lines):
    ...
    for line in lines[1:]:
        if has_no_text(line):
            ...
            #append line
            new_lines.append(timeStamp + line)
            global tSwitch
            tSwitch = True
    return new_lines
